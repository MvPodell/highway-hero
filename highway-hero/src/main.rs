...
impl engine::Game for Game {
    ...
    fn update(&mut self, engine: &mut Engine) {
        ...
        // if the number of sprites is less than 8, create a new apple and generate a new timer
        } else if engine.charas_by_tag(CharaTag::Car).count() < 5 {
            let car = engine.recycle_chara(
                self.spritesheet,
                CharaTag::Car,
                AABB {
                    center: Vec2 {
            ...
            engine[car].set_vel(Vec2 {
                x: 0.0,
                y: rng.gen_range((-4.0)..(-2.0)),
            });
            self.apple_timer = rng.gen_range(30..90);
        }
        let mut to_kill = vec![];
        // if an apple falls below the floor, mark it as "to kill"
        for (id, chara) in engine.charas_by_tag_mut(CharaTag::Car) {
            if chara.pos().y < -8.0 {
                to_kill.push(id);
            }
        }
    ...
    fn handle_triggers(
        ...
        for engine::Contact(_thing_a, tag_a, thing_b, tag_b, _amt) in triggers {
            ...
            if let (CharaTag::Guy, CharaTag::Car) = (tag_a, tag_b) {
                engine.kill_chara(thing_b);
                // increment score when collision
                self.score += 1;
            }
            // if let (CharaTag::Car, CharaTag::Car) = (tag_a, tag_b) {
            //     println!("car collision!");
            // }
        }
    }
